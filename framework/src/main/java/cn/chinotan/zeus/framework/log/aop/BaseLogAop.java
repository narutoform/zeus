package cn.chinotan.zeus.framework.log.aop;
...
import cn.chinotan.zeus.framework.util.AnsiUtil;
...
import cn.chinotan.zeus.framework.util.Jackson;
...
import cn.chinotan.zeus.config.properties.ZeusAopProperties;
...
public abstract class BaseLogAop {
     ...
     * Aop日志配置
     */
    protected ZeusAopProperties.LogAopConfig logAopConfig;

    /**
     * 日志打印类型
     */
    protected ZeusAopProperties.LogPrintType logPrintType;
     ...
     * requestId生成类型
     */
    protected ZeusAopProperties.RequestIdType requestIdType;

    /**
     * Aop操作日志配置
     */
    protected ZeusAopProperties.OperationLogConfig operationLogConfig;

    /**
     * Aop登录日志配置
     */
    protected ZeusAopProperties.LoginLogConfig loginLogConfig;

    @Autowired
    public void setZeusAopProperties(ZeusAopProperties zeusAopProperties) {
        logAopConfig = zeusAopProperties.getLog();
        ...
        operationLogConfig = zeusAopProperties.getOperationLog();
        loginLogConfig = zeusAopProperties.getLoginLog();
    ...
    protected void handleRequestId(RequestInfo requestInfo) {
        ...
        if (ZeusAopProperties.RequestIdType.IDWORK == requestIdType) {
            requestId = IdWorker.getIdStr();
        } else if (ZeusAopProperties.RequestIdType.UUID == requestIdType) {
    ...
    protected void handleRequestInfo(RequestInfo requestInfo) {
        ...
        if (ZeusAopProperties.LogPrintType.NONE == logPrintType) {
            return;
        }
        // 获取请求信息
        String requestInfoString = formatRequestInfo(requestInfo);
        // 如果打印方式为顺序打印，则直接打印，否则，保存的threadLocal中
        if (ZeusAopProperties.LogPrintType.ORDER == logPrintType) {
    ...
    protected void handleResponseResult(Object result) {
        ...
        if (result != null && result instanceof ApiResult) {
            ...
            if (ZeusAopProperties.LogPrintType.ORDER == logPrintType) {
                ...
                if (ZeusAopProperties.LogPrintType.LINE == logPrintType) {
                    printRequestInfoString(requestInfoString);
                    printResponseResult(code, responseResultString);
                } else if (ZeusAopProperties.LogPrintType.MERGE == logPrintType) {
    ...
    protected void saveSysOperationLog(RequestInfo requestInfo, OperationLogInfo operationLogInfo, Object result, Exception exception) {
        try {
            ...
            if (exception != null) {
                ...
                if (exception instanceof ZeusException) {
                    ZeusException zeusException = (ZeusException) exception;
                    errorCode = zeusException.getErrorCode();
                }
                // 异常字符串长度截取
